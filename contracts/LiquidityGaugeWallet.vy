...
def initialize(_owner: address, _lp_token: address, _atoken: address) -> bool:
    ...
    @notice _owner Address that owns the `lp_token` balance stored in this contract
    @notice _lp_token Curve LP token address
    @notice _atoken Address of Aave derivative token representing `lp_token`
    """
